      ...
      <div v-else>
        <div v-if="loginSuccess" class="shadow-2xl">
          ...
          >
            ...
            <div class="">
              <router-link
                v-if="this.user.administrator.member == true"
                :to="{ name: 'members_inbox' }"
              ...
              <router-link
                v-if="
                  this.user.administrator.departmentId != 19 &&
                  this.user.administrator.member != true
                "
                :to="{ name: 'dashboard' }"
                class="py-2 px-8 border border-transparent shadow-sm text-sm font-medium rounded-md text-green-600 border-green-600 hover:bg-green-600 hover:text-white duration-300 focus:outline-none"
              >
      ...
      this.$http.securityService
        ...
        .then((res) => {
          setTimeout(() => {
            ...
            localStorage.setItem("member", this.user.administrator.member);
            ...
            localStorage.setItem("dep_type", res.data.user.officeOrdirectorate);
            if (this.branchdepartmentNameSelected === "الأمانة الإدارية") {
              sessionStorage.setItem("dep_name2", this.departmentNameSelected);
            } else {
            ...
            setTimeout(() => {
              if (this.user.administrator.member == true) {
                this.$router.push({ name: "members_inbox" });
              } else if (this.user.administrator.departmentId == 19) {
